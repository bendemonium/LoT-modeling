...
def center_embedded(lexicon):
	...
	pf.add(basis, current)
	pf.remove(lexicon, current)
	buffer = pf.find(lexicon, current, chunker, move=True)
	current.purge()
	while(buffer):
		pf.loop()
		current = pf.sample(buffer)
...
def tail_recursive(lexicon): # head-tail
	...
	while(lexicon):
		pf.loop()
		base = pf.push_out(basis)
		buffer = pf.find(lexicon, base, bias, move=True)
		current = pf.sample(lexicon)
		buffer.purge()
		pf.remove(lexicon, current)
		pf.add(basis, base)
		...
		while(buffer):
			pf.loop()
			base = pf.push_out(basis)
			pf.write_random(buffer, base, bias)
			pf.add(basis, base)
			base.purge()
		buffer.purge()
	return(sequence)
